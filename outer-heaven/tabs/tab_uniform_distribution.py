...
import os
from models.uniform_distribution import UniformDistribution
from models.pseudo_random_number_visualizer import PseudoRandomNumberVisualizer

class TabUniformDistribution(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        title = tk.Label(self, text="DISTRIBUCIÓN UNIFORME", font=("Arial", 16, "bold")) 
        ...
        self.min_label.grid(row=1, column=0, pady=(0, 0))
        self.min_entry = tk.Entry(self, validate="key", validatecommand=vcmd)
        self.min_entry.grid(row=1, column=0, pady=(50, 0))
        
        self.max_label = tk.Label(self, text="Max:")  
        self.max_label.grid(row=1, column=0, pady=(100, 0))
        self.max_entry = tk.Entry(self, validate="key", validatecommand=vcmd)
        self.max_entry.grid(row=1, column=0, pady=(150, 0))
    ...
    def validate_fields(self):
        if not self.min_entry.get() or not self.max_entry.get() or not self.iteration_entry.get():
    ...
    def generate_numbers(self):
        ...
        generator = UniformDistribution(min_value, max_value, num_iterations)
